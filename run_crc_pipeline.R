...
tmp_local <- file.path(getwd(), "tmp")
dir.create(tmp_local, recursive = TRUE, showWarnings = FALSE)
Sys.setenv(TMPDIR = tmp_local, TEMP = tmp_local, TMP = tmp_local)

lib_local <- file.path(getwd(), "r_libs")
dir.create(lib_local, recursive = TRUE, showWarnings = FALSE)
.libPaths(c(lib_local, .libPaths()))

# ─────────────────────────────────────────────
# [STEP 0]  패키지 자동 설치
# ─────────────────────────────────────────────
cat("[0] Checking and installing required packages...\n")

# CRAN 패키지
cran_pkgs <- c("data.table", "dplyr", "tidyr", "vegan",
               "pROC", "PRROC", "randomForest", "xgboost",
               "ggplot2", "patchwork", "scales", "jsonlite")
...
for (pkg in bioc_pkgs) {
  if (!requireNamespace(pkg, quietly = TRUE)) {
    cat(sprintf("  Installing %s via BiocManager...\n", pkg))
...
dev_se_all <- curatedMetagenomicData(
  "ZellerG_2014.relative_abundance",
  dryrun = FALSE,
  counts  = TRUE
)[[1]]

# CRC + control 만 사용 (adenoma 등 제외)
...
ext_se_all <- curatedMetagenomicData(
...
# 코호트 정보 저장
cohort_info <- list(
  dev = list(n_total = nrow(dev_counts),
             n_crc   = sum(dev_y == 1),
             n_ctrl  = sum(dev_y == 0),
             n_taxa  = ncol(dev_counts)),
  ext = list(n_total = nrow(ext_counts),
             n_crc   = sum(ext_y == 1),
             n_ctrl  = sum(ext_y == 0),
             n_taxa  = ncol(ext_counts))
)
...
safe_rarefy <- function(counts_mat, depth) {
  ...
  keep <- rs >= depth
  if (!all(keep))
    message(sprintf("  [WARN] %d samples dropped (reads < %d)", sum(!keep), depth))
...
safe_prob_numeric <- function(x) {
  if (is.matrix(x) || is.data.frame(x)) {
    if (ncol(x) != 1L) {
      stop("Probability output must be a single column or vector.")
    }
    x <- x[, 1]
  }
  as.numeric(x)
...
if (length(miss_taxa) > 0) {
  message(sprintf("  [WARN] EXT missing %d taxa from DEV; padding with 0.", length(miss_taxa)))
...
cat("  [6.1] Logistic Regression (glm binomial)...\n")
df_tr  <- data.frame(y = y_train, X_train)
lr_fit <- glm(y ~ ., data = df_tr, family = binomial())

lr_p_train <- safe_prob_numeric(predict(lr_fit, data.frame(X_train), type = "response"))
lr_p_test  <- safe_prob_numeric(predict(lr_fit, data.frame(X_test),  type = "response"))
lr_p_ext   <- safe_prob_numeric(predict(lr_fit, data.frame(ext_clr), type = "response"))
cat("    Done.\n")

## 6-2. Random Forest (ntree=500)
cat("  [6.2] Random Forest (ntree=500)...\n")
set.seed(SEED)
mtry0  <- max(1L, floor(sqrt(ncol(X_train))))
rf_fit <- randomForest(x = X_train, y = as.factor(y_train),
                       ntree = 500, mtry = mtry0, importance = TRUE)

rf_p_train <- safe_prob_numeric(predict(rf_fit, X_train,  type = "prob")[, "1", drop = FALSE])
rf_p_test  <- safe_prob_numeric(predict(rf_fit, X_test,   type = "prob")[, "1", drop = FALSE])
rf_p_ext   <- safe_prob_numeric(predict(rf_fit, ext_clr,  type = "prob")[, "1", drop = FALSE])
cat("    Done.\n")

## 6-3. XGBoost (max_depth=6, eta=0.1, nrounds=300)
cat("  [6.3] XGBoost (nrounds=300)...\n")
...
xgb_p_train <- safe_prob_numeric(predict(xgb_fit, dtrain))
xgb_p_test  <- safe_prob_numeric(predict(xgb_fit, dtest))
xgb_p_ext   <- safe_prob_numeric(predict(xgb_fit, dext))
cat("    Done.\n\n")

# ─────────────────────────────────────────────
# [STEP 7]  임계값 선택 (Youden J)
# ─────────────────────────────────────────────
cat("[7] Selecting optimal thresholds (Youden J on train set)...\n")

youden_threshold <- function(y_true, p_hat) {
  p_hat <- safe_prob_numeric(p_hat)
  roc_obj <- pROC::roc(y_true, p_hat, quiet = TRUE, direction = "<")
  as.numeric(pROC::coords(roc_obj, "best", best.method = "youden",
...
calibration_ece <- function(y_true, p_hat, n_bins = 10) {
  p_hat <- safe_prob_numeric(p_hat)
  bins <- cut(p_hat, breaks = seq(0, 1, length.out = n_bins + 1),
...
evaluate_model <- function(name, y_true, p_hat, threshold, boot_n = 2000) {
  ...
  f1     <- ifelse(is.na(recall) | is.na(prec) | (prec + recall) == 0,
                   NA_real_, 2 * prec * recall / (prec + recall))
  brier  <- mean((p_hat - y_true)^2)
  cal    <- calibration_ece(y_true, p_hat, N_BINS_CAL)

  data.frame(Model = name,
             AUC = round(auc_val, 4), AUC_CI_L = round(ci[1], 4),
             AUC_CI_U = round(ci[3], 4), PR_AUC = round(pr_auc, 4),
             Accuracy = round(acc, 4), Recall = round(recall, 4),
             Precision = round(prec, 4), F1 = round(f1, 4),
             Brier = round(brier, 4), ECE = round(cal$ece, 4),
             Threshold = round(threshold, 4),
             stringsAsFactors = FALSE)
}

# ─────────────────────────────────────────────
# [STEP 9]  성능 평가 (Bootstrap CI — 약 2-5분 소요)
# ─────────────────────────────────────────────
...
cat(sprintf("  DeLong DEV XGB vs LR: ΔAUC=%+.4f, p=%.4f\n",
            diff(dl_xlr_dev$estimate), dl_xlr_dev$p.value))
cat(sprintf("  DeLong DEV XGB vs RF: ΔAUC=%+.4f, p=%.4f\n",
            diff(dl_xrf_dev$estimate), dl_xrf_dev$p.value))
cat(sprintf("  DeLong EXT XGB vs LR: ΔAUC=%+.4f, p=%.4f\n",
            diff(dl_xlr_ext$estimate), dl_xlr_ext$p.value))
...
make_roc_df <- function(y_true, p_hat, model_name) {
  ...
  coords  <- pROC::coords(roc_obj, "all",
                           ...
                           transpose = FALSE)
  data.frame(FPR   = 1 - coords$specificity,
             TPR   = coords$sensitivity,
             Model = sprintf("%s (AUC=%.3f)", model_name, auc_val))
}

roc_dev_df <- bind_rows(make_roc_df(y_test, lr_p_test,  "LR"),
                        make_roc_df(y_test, rf_p_test,  "RF"),
                        make_roc_df(y_test, xgb_p_test, "XGB"))
roc_ext_df <- bind_rows(make_roc_df(ext_y2, lr_p_ext,  "LR"),
...
plot_roc <- function(df, title) {
  ggplot(df, aes(FPR, TPR, color = Model)) +
    ...
    theme(legend.position = "inside", legend.position.inside = c(0.65, 0.18),
          plot.title = element_text(face = "bold"))
}

fig1 <- plot_roc(roc_dev_df, "Development Cohort (Test Set)") +
        plot_roc(roc_ext_df, "External Validation Cohort")
ggsave(file.path(OUTDIR, "Figure1_ROC_curves.png"), fig1, width = 13, height = 5.5, dpi = 300)
cat("  Figure 1 (ROC curves) saved.\n")
...
fig2 <- ggplot(cal_df, aes(p_mean, y_rate, color = Model, group = Model)) +
  ...
  labs(title = "Calibration Reliability Curves",
       x = "Mean Predicted Probability", y = "Observed Event Rate", color = NULL) +
  theme_minimal(base_size = 12) +
...
metrics_long <- bind_rows(
  ...
  pivot_longer(cols = c(AUC, PR_AUC, Recall, F1, Accuracy),
...
fig3 <- ggplot(metrics_long, aes(Metric, Value, fill = Model)) +
  ...
  geom_text(aes(label = sprintf("%.3f", Value)),
            position = position_dodge(0.75), vjust = -0.4, size = 2.5, fontface = "bold") +
  scale_fill_manual(values = model_colors) +
  scale_y_continuous(limits = c(0, 1.12), labels = scales::percent) +
  facet_wrap(~ Cohort) +
  labs(title = "Model Performance Comparison", x = NULL, y = "Score", fill = NULL) +
  theme_minimal(base_size = 12) +
...
df_rf_imp <- data.frame(
  ...
  Importance = as.numeric(top_rf), Model = "Random Forest")
...
fig4 <- (ggplot(df_rf_imp, aes(Importance, Taxa)) +
           geom_bar(stat = "identity", fill = model_colors["RF"], alpha = 0.85) +
           labs(title = "Random Forest\n(Mean Decrease Accuracy)", x = "Importance", y = NULL) +
           theme_minimal(base_size = 10) +
           theme(plot.title = element_text(face = "bold", size = 10))) +
        (ggplot(df_xgb_imp, aes(Importance, Taxa)) +
           geom_bar(stat = "identity", fill = model_colors["XGB"], alpha = 0.85) +
           labs(title = "XGBoost\n(Gain)", x = "Importance", y = NULL) +
           theme_minimal(base_size = 10) +
           theme(plot.title = element_text(face = "bold", size = 10))) +
...
# ─────────────────────────────────────────────
# [STEP 13]  JSON 결과 저장 (문서 자동 업데이트용)
# ─────────────────────────────────────────────
cat("\n[13] Saving JSON summary for document update...\n")

final_json <- list(
  cohort_info = cohort_info,
  split_info  = list(
    train_n   = length(y_train),
    train_crc = sum(y_train == 1),
    train_ctrl= sum(y_train == 0),
    test_n    = length(y_test),
    test_crc  = sum(y_test == 1),
    test_ctrl = sum(y_test == 0),
    n_taxa    = ncol(dev_clr)
  ),
  thresholds = list(LR = lr_thr, RF = rf_thr, XGB = xgb_thr),
  dev_results = res_dev,
  ext_results = res_ext,
  statistical_tests = list(
    delong_dev_xgb_vs_lr = list(
      delta_auc = round(diff(dl_xlr_dev$estimate), 4),
      p_value   = round(dl_xlr_dev$p.value, 4)
    ),
    delong_dev_xgb_vs_rf = list(
      delta_auc = round(diff(dl_xrf_dev$estimate), 4),
      p_value   = round(dl_xrf_dev$p.value, 4)
    ),
    delong_ext_xgb_vs_lr = list(
      delta_auc = round(diff(dl_xlr_ext$estimate), 4),
      p_value   = round(dl_xlr_ext$p.value, 4)
    ),
...
write(jsonlite::toJSON(final_json, pretty = TRUE, auto_unbox = TRUE),
      file.path(OUTDIR, "final_results.json"))
cat("  JSON saved.\n")
...
cat("========================================\n")
cat("\n[Development Cohort Test Set]\n")
print(res_dev[, c("Model","AUC","AUC_CI_L","AUC_CI_U","PR_AUC","Recall","F1","Brier","ECE")])
cat("\n[External Validation Cohort]\n")
print(res_ext[, c("Model","AUC","AUC_CI_L","AUC_CI_U","PR_AUC","Recall","F1","Brier","ECE")])
cat("\n[Statistical Tests]\n")
cat(sprintf("  DeLong  DEV XGB vs LR: ΔAUC=%+.4f, p=%.4f\n",
            diff(dl_xlr_dev$estimate), dl_xlr_dev$p.value))
cat(sprintf("  DeLong  DEV XGB vs RF: ΔAUC=%+.4f, p=%.4f\n",
...
cat(sprintf("\n모든 결과가 저장되었습니다: %s\n", OUTDIR))
cat("========================================\n")
cat("\n완료! Claude에게 결과 확인을 요청하세요.\n")

# ─────────────────────────────────────────────────────────────────────────────
# [EXTRA] add_figures.py 호환 파일 저장
#   → run_analysis.R 결과를 generate_figures.R / add_figures.py 와 연결
# ─────────────────────────────────────────────────────────────────────────────
cat("\n[EXTRA] add_figures.py 호환 파일 저장 중...\n")

FIG_DIR <- file.path(OUTDIR, "figures")
dir.create(FIG_DIR, showWarnings = FALSE, recursive = TRUE)

# 1. ROC 객체 저장 (generate_figures.R 에서 정확한 ROC 곡선 생성용)
roc_rf_e <- pROC::roc(ext_y2, rf_p_ext, quiet = TRUE, direction = "<")
saveRDS(
  list(
    dev = list(LR = roc_lr_d, RF = roc_rf_d, XGB = roc_xgb_d),
    ext = list(LR = roc_lr_e, RF = roc_rf_e, XGB = roc_xgb_e)
  ),
  file.path(OUTDIR, "roc_curves.rds")
)
cat("  roc_curves.rds 저장됨\n")

# 2. XGBoost feature importance 저장
xgb_imp_df <- xgb.importance(model = xgb_fit)
saveRDS(xgb_imp_df, file.path(OUTDIR, "xgb_importance.rds"))
cat("  xgb_importance.rds 저장됨\n")

# 3. 기존 그래프를 figures/ 폴더에 표준 이름으로 복사
fig_rename <- list(
  "Figure1_ROC_curves.png"         = "fig1_roc_dev.png",
  "Figure2_Calibration.png"        = "fig_cal.png",
...
for (src_name in names(fig_rename)) {
  src <- file.path(OUTDIR, src_name)
  dst <- file.path(FIG_DIR, fig_rename[[src_name]])
  if (file.exists(src)) {
    file.copy(src, dst, overwrite = TRUE)
    cat(sprintf("  %s → figures/%s\n", src_name, fig_rename[[src_name]]))
  }
}

# fig2_roc_ext 는 generate_figures.R 에서 별도 생성됨 (ROC 외부 단독)
# generate_figures.R 을 실행하면 fig2_roc_ext.png 가 추가로 생성됩니다

cat("\n[완료] 그래프 삽입 준비 완료!\n")
cat("다음 단계:\n")
cat("  1. RStudio: source('R_scripts/generate_figures.R')\n")
cat("  2. 명령 프롬프트: python add_figures.py\n")
cat(sprintf("  그래프 폴더: %s\n", FIG_DIR))
